"""Asynchronous file writers for pose_act jitter diagnostics.

Three files may be produced under a configured `jitter_dump_dir`:

- ``chunk_dump.jsonl`` (server-side): one line per inference, raw absolute pose7d chunk.
- ``aggregate_events.jsonl`` (client-side): one line per overlapping-timestep fusion event.
- ``executed.csv`` (client-side): one line per action popped from the local queue and
  sent to the robot, in both pre-adapter and post-adapter forms.
...
_CHUNK_DUMP_NAME = "chunk_dump.jsonl"
_AGG_EVENTS_NAME = "aggregate_events.jsonl"
_EXECUTED_CSV_NAME = "executed.csv"
...
class JitterDumpWriter:
    ...
    def write_chunk(
        ...
        *,
        ...
        _writer.submit_jsonl(self.dump_dir / _CHUNK_DUMP_NAME, payload)
    ...
    def write_aggregate_event(
        ...
        *,
        ...
        _writer.submit_jsonl(self.dump_dir / _AGG_EVENTS_NAME, payload)
    ...
    def write_executed(
        ...
        *,
        ...
        _writer.submit_csv(self.dump_dir / _EXECUTED_CSV_NAME, header, row)
