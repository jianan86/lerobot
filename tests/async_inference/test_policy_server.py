...
def test_umi_pi05_openpi_checkpoint_builds_processors_from_mapped_stats(monkeypatch, tmp_path):
    from lerobot.async_inference.configs import PolicyServerConfig
    from lerobot.async_inference.helpers import RemotePolicyConfig
    from lerobot.async_inference.policy_server import PolicyServer
    from lerobot.policies.umi_pi05 import UmiPI05Config
    from lerobot.transport import services_pb2
    from lerobot.utils.constants import ACTION
    ...
    class PolicyStub:
        def __init__(self):
            self.config = UmiPI05Config(device="cpu")
            self.config.input_features = {OBS_STATE: PolicyFeature(type=FeatureType.STATE, shape=(20,))}
            self.config.output_features = {ACTION: PolicyFeature(type=FeatureType.ACTION, shape=(10, 20))}
    ...
    def fake_umi_processors(config, dataset_stats=None):
        captured["config"] = config
        captured["stats"] = dataset_stats
        return "pre", "post"

    def fail_saved_processors(*_args, **_kwargs):
        raise AssertionError("saved processor loader should not be used for OpenPI UMI checkpoints")

    monkeypatch.setattr("lerobot.async_inference.policy_server.get_policy_class", lambda _name: PolicyStub)
    monkeypatch.setattr("lerobot.async_inference.policy_server.make_umi_pi05_pre_post_processors", fake_umi_processors)
    monkeypatch.setattr("lerobot.async_inference.policy_server.make_pre_post_processors", fail_saved_processors)
    ...
    assert set(captured["stats"]) == {OBS_STATE, ACTION}
