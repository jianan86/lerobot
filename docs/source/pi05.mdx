...
To load the PaliGemma tokenizer from a local directory instead of the Hugging Face Hub, add:

```bash
--policy.tokenizer_name_or_path=/data/jianan/weight/paligemma-3b-pt-224
```
...
| `freeze_language_model` | `false` | Do not freeze the PaliGemma language model        |
| `train_expert_only`     | `false` | Do not freeze the VLM, train all parameters       |

**💡 Tip**: Setting `train_expert_only=true` freezes the VLM and trains only the action expert and projections, allowing finetuning with reduced memory usage.

To freeze only the language model while training the vision encoder, multi-modal projector, and action expert, set `freeze_language_model=true`, `freeze_vision_encoder=false`, and `train_expert_only=false`.
